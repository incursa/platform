param(
    [string]$SpecsRoot = "specs/providers",
    [string]$MatrixPath = "specs/providers/provider-conformance-matrix.md",
    [string]$SummaryPath = "artifacts/codex/provider-traceability-summary.md"
)
...
$specIdsByFile = [ordered]@{}
$allSpecIds = New-Object 'System.Collections.Generic.HashSet[string]' ([StringComparer]::Ordinal)

foreach ($specFile in $specFiles) {
    $fullPath = Join-Path $specsRootPath $specFile
    if (-not (Test-Path $fullPath)) {
        Write-Error "Spec file not found: $fullPath"
        exit 1
    }

    $content = Get-Content $fullPath -Raw
    $matches = [regex]::Matches($content, 'PRIM-[A-Z0-9]+-[A-Z0-9]+-\d{3}')
...
foreach ($line in Get-Content $matrixFullPath) {
    $lineNumber++
    if ($line -match '^\|\s*(PRIM-[A-Z0-9]+-[A-Z0-9]+-\d{3})\s*\|\s*([^|]+)\|\s*([^|]+)\|\s*(Covered|Missing|Deferred)\s*\|\s*(.*?)\s*\|$') {
...
$summary.Add("# Provider Traceability Summary")
$summary.Add("")
