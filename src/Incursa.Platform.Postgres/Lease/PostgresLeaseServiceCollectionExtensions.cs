...
/// </summary>
internal static class PostgresLeaseServiceCollectionExtensions
{
    /// <summary>
    ...
    public static IServiceCollection AddSystemLeases(this IServiceCollection services, PostgresSystemLeaseOptions options)
    {
        ...
        services.Configure<PostgresSystemLeaseOptions>(o =>
        {
    ...
    /// <returns>The IServiceCollection so that additional calls can be chained.</returns>
    [Obsolete("This method uses a hardcoded connection string and bypasses dynamic discovery. Use AddPostgresPlatformMultiDatabaseWithDiscovery or AddPostgresPlatformMultiDatabaseWithList instead to ensure all databases go through IPlatformDatabaseDiscovery.")]
    public static IServiceCollection AddSystemLeases(this IServiceCollection services, string connectionString, string schemaName = "infra")
    {
        ...
        var options = new PostgresSystemLeaseOptions
        {
        ...
        {
            var logger = provider.GetRequiredService<ILogger<PostgresLeaseFactory>>();
            return new PostgresLeaseFactory(leaseFactoryConfig, logger);
        });
    ...
    /// <returns>The IServiceCollection so that additional calls can be chained.</returns>
    [Obsolete("This method uses hardcoded database configurations and bypasses dynamic discovery. Use AddPostgresPlatformMultiDatabaseWithDiscovery or AddPostgresPlatformMultiDatabaseWithList instead to ensure all databases go through IPlatformDatabaseDiscovery.")]
