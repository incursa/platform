...
internal sealed class PostgresAuditOptionsValidator : IValidateOptions<PostgresAuditOptions>
{
    public ValidateOptionsResult Validate(string? name, PostgresAuditOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.AuditEventsTable))
        {
            failures.Add("AuditEventsTable is required.");
        }

        if (string.IsNullOrWhiteSpace(options.AuditAnchorsTable))
        {
            failures.Add("AuditAnchorsTable is required.");
        }
