...
internal sealed class PlatformFanoutRepositoryProvider : IFanoutRepositoryProvider, IDisposable
{
    ...
    private async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var options = new PostgresFanoutOptions
                {
                    ConnectionString = db.ConnectionString,
                    SchemaName = db.SchemaName,
                };

                var policyRepo = new PostgresFanoutPolicyRepository(Options.Create(options));
                var cursorRepo = new PostgresFanoutCursorRepository(Options.Create(options));
