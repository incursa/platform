...
internal class SchedulerHealthCheck : IHealthCheck
{
    ...
    public SchedulerHealthCheck(IOptions<PostgresSchedulerOptions> options, TimeProvider timeProvider)
    {
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            var outboxTable = PostgresSqlHelper.Qualify(schemaName, "Outbox");
            var timersTable = PostgresSqlHelper.Qualify(schemaName, timersTableName);
            var jobRunsTable = PostgresSqlHelper.Qualify(schemaName, jobRunsTableName);
