...
/// <summary>
/// Provides utility methods for comparing PostgreSQL connection strings.
/// </summary>
internal static class ConnectionStringComparer
{
    /// <summary>
    /// Compares two PostgreSQL connection strings for equivalence by parsing and comparing
    ...
    public static bool AreEquivalent(string? connectionString1, string? connectionString2)
    {
        ...
        try
        {
            var builder1 = new NpgsqlConnectionStringBuilder(connectionString1);
            var builder2 = new NpgsqlConnectionStringBuilder(connectionString2);

            // Compare the key components that define database identity
            return string.Equals(builder1.Host, builder2.Host, StringComparison.OrdinalIgnoreCase) &&
                   builder1.Port == builder2.Port &&
                   string.Equals(builder1.Database, builder2.Database, StringComparison.OrdinalIgnoreCase);
        }
