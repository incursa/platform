...
internal sealed class SqlInboxOptionsValidator : IValidateOptions<SqlInboxOptions>
{
    public ValidateOptionsResult Validate(string? name, SqlInboxOptions options)
    {
        ...
        {
            failures.Add("MaxAttempts must be greater than zero.");
        }

        if (options.LeaseSeconds <= 0)
        {
            failures.Add("LeaseSeconds must be greater than zero.");
        }
