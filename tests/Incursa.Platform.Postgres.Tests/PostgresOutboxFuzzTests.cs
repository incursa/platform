...
public sealed class PostgresOutboxFuzzTests : PostgresTestBase
{
    ...
    public PostgresOutboxFuzzTests(ITestOutputHelper testOutputHelper, PostgresCollectionFixture fixture)
    ...
    /// <behavior>Completed/failed terminal items are never returned by subsequent claims.</behavior>
    [Fact]
    public async Task Outbox_FuzzDeterministic_TerminalItemsAreNeverReclaimed()
    {
        var seeded = new DeterministicSequence(2026U);
    ...
    /// <behavior>Claims in each round do not overlap and terminal ids are never reclaimed later.</behavior>
    [Fact]
    public async Task Outbox_FuzzConcurrentRounds_ClaimsStayDisjointAndTerminalItemsAreNotReclaimed()
    {
        var seeded = new DeterministicSequence(9001U);
        var terminal = new HashSet<OutboxWorkItemIdentifier>();
