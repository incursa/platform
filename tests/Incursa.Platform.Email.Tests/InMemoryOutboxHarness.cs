...
internal sealed class InMemoryOutboxHarness : IOutbox, IOutboxStore
{
    ...
    private OutboxMessage CreateMessage(Entry entry)
    {
        ...
        SetProperty(message, nameof(OutboxMessage.Id), CreateWorkItemIdentifier(entry.Id));
        SetProperty(message, nameof(OutboxMessage.CreatedAt), timeProvider.GetUtcNow());
        SetProperty(message, nameof(OutboxMessage.RetryCount), entry.RetryCount);
        SetProperty(message, nameof(OutboxMessage.CorrelationId), entry.CorrelationId);
        SetProperty(message, nameof(OutboxMessage.DueTimeUtc), entry.DueTimeUtc);
    ...
    private static void SetProperty<T>(OutboxMessage message, string propertyName, T value)
    {
        var property = typeof(OutboxMessage).GetProperty(propertyName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
