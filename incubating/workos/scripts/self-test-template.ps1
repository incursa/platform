...
try {
    ...
    try {
        ...
        Invoke-External -Description "dotnet test" -FilePath "dotnet" -Arguments @("test", "--solution", $solution.Name, "-c", "Release", "--no-build", "--filter", "TestCategory!=Integration&TestCategory!=Fuzz&RequiresDocker!=true")
