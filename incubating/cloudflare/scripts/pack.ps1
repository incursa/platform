...
if (-not $SkipRestore) {
    & dotnet restore $solution.Name -p:NuGetAudit=false
...
if (-not $SkipBuild) {
    & dotnet build $solution.Name -c $Configuration --no-restore -p:NuGetAudit=false -p:ContinuousIntegrationBuild=true
    if ($LASTEXITCODE -ne 0) {
